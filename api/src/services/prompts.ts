...
- 3–10 interactions total, scaled to length (§8.1 "When to interact" — where a residual "3–7" figure survives elsewhere in the playbook (§1's Hard Rules, §4's global limits), §8.1's 3–10 PREVAILS); never more than 30 seconds of running time without one; NONE during the title card; placement per the §8.1 skeleton (intro 0–1 · 1 check after the full model, or 1 per chunk of a chunked model · 2–6 We Do micro-checks · 1–2 instant checks in the discrimination pass, which lives inside late We Do / the wrap per §5's Part C mapping · wrap 0–1 instant). The per-segment maxima never justify exceeding the 3–10 total.
- The intro NEVER reviews preskills or prerequisites (§4). It names the skill in student terms, sets the goal, and introduces the concept — matched to what the lesson introduces: a rule or vocabulary lesson STATES the rule or DEFINES the term, with an example when applicable, BEFORE the first problem; an identification lesson shows examples AND nonexamples with explicit contrast; a visual concept is SHOWN on the canvas, not described; when the lesson leans on specific vocabulary, cover it before jumping into the problem. Keep it tight — seconds, not a lecture.
- MCQ is a LAST RESORT (§1 Hard Rules, §8.1 "Options and answers"): default to a direct response that demonstrates the target skill (numeric/text entry, click-to-highlight, drag), and NEVER use multiple-choice as a worked example's response form (I Do confirm, We Do micro-steps) unless the lesson objective itself requires choosing between alternatives or a discrimination that cannot be measured more directly (§8.2's discrete-decision cases like "Do we regroup?" remain legitimate MCQs).
