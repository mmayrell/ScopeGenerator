...
Engine and doctrine sit outside this chain: they are the rules the evidence is processed under. Where doctrine sources or interpretive documents disagree on method, Stein's DI method prevails (P3). EXCEPTION THAT STRENGTHENS DOCTRINE: under the Stein Direct-Match doctrine (SDM, below), a lesson or skill with a DIRECT instructional match in Stein is governed by Stein for scope, order, grain, formats, examples, and instructional progression — subject only to level 1's boundary veto.`
...
// House doctrine (adopted 2026-07-18) — cited in Decision Records by the rule
// tag "SDM". Strengthens Stein's authority wherever the book DIRECTLY treats
// the skill being planned or carded; where no direct match exists it changes
// nothing (the other evidence and atomization principles govern).
const STEIN_MATCH = `THE STEIN DIRECT-MATCH DOCTRINE (SDM — binding on every generation decision; cite rule tag "SDM" in Decision Records):
When a lesson or skill has a DIRECT instructional match in Stein et al. (Direct Instruction Mathematics, 5th ed., 2017 — a teaching format, an Instructional Sequence and Assessment Chart row, or a chapter treatment of exactly this skill), Stein becomes the CONTROLLING AUTHORITY for that lesson's scope, order, grain, formats, examples, and instructional progression — subject ONLY to the standard's boundary (the P1 veto). Every SDM-matched lesson is TYPED stein-exact under the engine's Kinds of Lessons. Specifically:
- FOLLOW THE TREATMENT WHOLE: the tool follows every instructionally meaningful element of the matched treatment — the strategy and its step order, the preskills the format isolates, the example-selection mix and ratios, the problem-type sequence, the correction procedures, the documented error patterns and their scheduled emphasis. Each card field must align to Stein's corresponding structure, conditions, language, and task format, cited to the printed page.
- FORMATS ARE NEVER COLLAPSED: distinct Stein formats are distinct atoms — each format becomes its own lesson. Two formats never merge into one lesson because they "feel like one skill"; the format boundary IS the atom boundary (a named A2 criterion is not required to keep matched formats apart — SDM itself is the split authority there).
- DEFERRAL IS A SPLIT SIGNAL (top priority): any direction in Stein to defer a problem type or format, introduce it later, return to it after an interval, or schedule delayed emphasis is an AUTOMATIC lesson-split signal — the deferred material becomes its own later lesson (or the parent's explicitly scheduled Delayed Modeling Case ONLY when Stein itself keeps it inside the same exercise). This signal outranks the Don't-Split rules wherever the two collide.
- THE EXEMPLAR PROVES THE MATCH: every SDM-matched lesson carries a concrete exemplar item that faithfully INSTANTIATES Stein's task format (the format's own problem shape, response form, and conditions) so the alignment is inspectable — as a generated exemplar when no released item is placed, or noted against the placed item that already instantiates it.
- NEVER STRETCH A MATCHED LESSON: when a released item remains in scope but demands a more advanced format or level of application than the Stein-matched lesson teaches, the matched lesson is NOT stretched or rewritten to absorb it. Create a separate SUBSEQUENT lesson — typically typed test-rigor, or a sequence of bridge lessons — that scaffolds from Stein's prescribed endpoint to the released item's demand, and place the item there (the Deferral Rule and No-Forcing Rule apply as usual).
- WHERE NO DIRECT STEIN LESSON EXISTS, the other evidence and atomization principles govern unchanged — SDM never manufactures a match, and a doctrine-consistent generalization of a nearby format is flagged as such (never presented as a direct match).`
...
- A4 Kinds of Lessons & Doctrines: type every lesson as stein-exact (the exact lesson is described in Stein's book — a direct instructional match: a named teaching format, an Instructional Sequence and Assessment Chart row, or a chapter treatment of exactly this skill; SDM controls it) | new-learning (teaches one new behavior, defined by the atom triple) | test-rigor (inserted to explicitly provide students state testing rigor — assessment evidence demands a performance level or coordination the teaching sequence alone would not reach, including the scaffold from a Stein-matched endpoint to a released item's demand) | bridge (inserted where a split pair is confusable — the new learning is the discrimination itself: recognize from the first cue which routine applies, execute it cleanly without blending; mixed look-alike practice engineered to trigger the confusion, no new rules modeled) | application-tier (executes an already-mastered routine in a new demand band — multi-step contexts, interpret-and-explain; boundary and ceiling inherit from the parent atom). The kind records instructional PURPOSE, never importance/duration/position — A1 discovery is unchanged, so preskill, representation, vocabulary/concept, and selection atoms still exist as atoms and type stein-exact when Stein directly treats them, otherwise new-learning. Representation Doctrine: representations are instructional only when students must learn to interpret, construct, or choose them — otherwise they remain examples inside another lesson. Vocabulary Doctrine: words are never taught in isolation; vocabulary lessons exist only when the word introduces a new concept or misunderstanding it blocks later instruction. Bridge Doctrine: bridges never teach new mathematics; the title contains the competing atoms ("A vs. B") or the explicit selection behavior, never "Mixed Practice" or "Review"; placed only after both parents are independently mastered. Application Doctrine: applications never introduce new computational procedures. Test Rigor Doctrine: test-rigor lessons never introduce new computational procedures — the new learning is performing already-taught content at the assessment's demand; earned by assessment evidence, never inserted on speculation.
...
10 approach — exactly one named strategy, selected per Stein (P3), followed by the lesson's ENTIRE within-lesson progression. This field specifies HOW the lesson's in-boundary content is taught — never WHAT the lesson includes (that is the boundary's job) — and must stay entirely within the Assessment Boundary and never exceed the Difficulty Ceiling. Begin stein-exact and new-learning approach fields with "Single strategy: <name>". REQUIRED Example Progression structure, in this order:
...
- The Coordination Rule: a multi-atom item (choosing between procedures, or coordinating several mastered atoms) may only be placed at a bridge, test-rigor, or application-tier lesson.
...
- The Exclusion Triage Rule (when an embedded demand never enters the ledger — mandatory order): Q1 prior-grade content? Activatable in under two minutes → add it to M(0) as a SEPARATE prereq entry flagged addedByTriage and re-place; needs explicit reteaching → insert a preskill lesson (typed stein-exact when Stein directly treats it, otherwise new-learning), rebuild the ledger, re-place. Q2 within the standard's scope but taught nowhere? The item exposed a missing atom — re-run discovery on that demand, insert the atom flagged "inserted-by-triage", rebuild the ledger, re-place all affected items (multiple items orphaned by the same demand are strong evidence the atom is missing). Q3 beyond the standard and the grade → EXCLUDE, silently: excluded items get NO itemRefs entry and NO annotation anywhere. The triage is the feedback loop through which item alignment audits the atomization itself.
...
CITING THE TEXTBOOK IS MANDATORY where these excerpts govern a decision. On every stein-exact and new-learning lesson whose topic the excerpts cover: (a) the approach field carries a doctrine citation, and (b) the strategy-selection decision entry (type "strategy", field "approach") carries a doctrine citation naming its Stein basis. Doctrine citations use sourceType "doctrine", label = the chapter title (e.g. "Direct Instruction Mathematics (5th ed.), ch. Division"), locator = the most specific location WITH ITS PRINTED PAGE — the teaching format number and name, the Instructional Sequence and Assessment Chart row, or the section heading — and excerpt = the verbatim sentence(s) of the excerpt that drove it. The same applies wherever the excerpts drive OTHER decisions: preskill choices they name (prerequisites), problem-type sequences they prescribe (progression, boundary, ceiling), documented error patterns and remediation-chart rows they inventory (newLearning, delayed modeling cases, distractor design) are cited the same way. Where the excerpts do not cover the specific case, fall back to the doctrine principles in the system prompt (cite the doctrine framework document itself) and say so in the rationale; a doctrine-consistent GENERALIZATION of a book format (e.g. parameterizing a tens-unit rounding format to any place) is flagged as such in the decision record — never presented as a verbatim doctrine prescription.
SDM APPLIES CARD-BY-CARD (the Stein Direct-Match doctrine, system prompt): when these excerpts DIRECTLY treat a lesson's skill (a matching format, chart row, or chapter treatment), Stein controls that card — every card field aligns to Stein's corresponding structure, conditions, language, and task format (boundary and ceiling framed in the format's own problem-type terms, subject only to the standard's boundary; newLearning's cue/decision path/response form the format's own; the approach's Modeled Set, Delayed Modeling Cases, and Vary/Hold Constant following the book's example-selection guidance), each alignment cited to the printed page and the strategy decision entry tagged "SDM". THE EXEMPLAR PROVES THE MATCH: an SDM-matched lesson with no placed item writes its required generated exemplar(s) to faithfully INSTANTIATE Stein's task format (the format's own problem shape, response form, and conditions — basis names the format and page); a matched lesson WITH placed items verifies in the releasedItems rationale that a placed item instantiates the format, and when none does, ADDS one labeled generated exemplar that does. Never stretch an SDM-matched card beyond its format's endpoint to absorb a harder item — that demand belongs to the subsequent/bridge lesson the plan created for it.
<doctrine_excerpts>
${excerpts}
</doctrine_excerpts>
`
...
STEIN DIRECT-MATCH PASS (SDM — run per standard, before the split decisions settle): identify every skill in this unit that the doctrine evidence treats DIRECTLY (a named teaching format, an Instructional Sequence and Assessment Chart row, or a chapter treatment of exactly that skill). For each direct match, SDM controls the grain and order: one Stein format = one atom (matched formats never collapse into a shared lesson — SDM itself is the split authority, no A2 criterion needed); any Stein direction to defer a problem type, introduce it later, return after an interval, or schedule delayed emphasis is an AUTOMATIC split signal that outranks the Don't-Split rules; the matched lessons follow Stein's problem-type sequence. When a released item in scope demands a more advanced format or application level than the matched lesson's Stein endpoint, NEVER stretch the matched lesson — create the separate subsequent lesson (or bridge-lesson sequence) that scaffolds from Stein's endpoint to the item's demand and place the item there (Deferral/No-Forcing as usual). Every direct match is TYPED stein-exact; the scaffold lessons inserted between a Stein endpoint and a released item's demand are typically typed test-rigor. Record every SDM call in planningNotes (rule tag "SDM", with the format/chart-row named) so card generation instantiates the match; where no direct Stein treatment exists, say nothing and let the other principles govern.
...
- lesson-type semantics (the engine's Kinds of Lessons): stein-exact newLearning = the behavior Stein's matched treatment teaches, the card instantiating the format whole under SDM (approach follows the format's strategy, step order, example mix, and correction procedures, cited to the printed page); new-learning newLearning = the one new behavior, defined by the atom triple (this covers atoms discovered as preskills, representations, or vocabulary/concept stabilizations — a representation atom's newLearning is the new mapping between mathematical meaning and the representation's form, its approach modeling the representation over already-mastered content; a preskill atom is the prerequisite skill itself, taught explicitly, never embedded in a later lesson); test-rigor newLearning = performing already-taught content at the state assessment's demand (its item conventions, response formats, cognitive coordination, precision — never a new computational procedure), earned by assessment evidence; bridge newLearning = the discrimination itself between the confusable split pair — recognize from the first cue which routine applies and execute it cleanly without blending, approach = mixed look-alike practice engineered to trigger the confusion with no new rules modeled (bridges never teach new mathematics); application-tier newLearning = executing the mastered routine(s) in the new demand band (multi-step contexts, interpret-and-explain, modeling, justifying, multi-atom coordination — never a new computational procedure), boundary/ceiling inherited from the parent atom(s) plus the triggering demand statement's scope.
...
${doctrine.chapterExcerpt}
  ...
  return {
    system: vsgSystem(),
    user: `Write the video script for the lesson below, following the rulebook's per-lesson pipeline (§16) IN ORDER: 1 assemble context (grade band "${gradeBand}") → 2 retrieve doctrine (supplied below) → 3 ENUMERATE CASE CLASSES the strategy claims within the card's boundary, marking taught vs deferred — this drives the Transfer Test and the coverage note (SEQ 08–SEQ 11) → 4 plan examples (first example mid-difficulty and unambiguous; further examples chosen to cover remaining case classes; discrimination examples where the boundary allows, SEQ 07; released items calibrate demand, never appear, SEQ 13) → 5 draft the opening (SEQ 15, SEQ 06) → 6 draft I Do from the format's Part A per §18 → 7 build We Do from the format's Part B/C question sequence → 8 write the wrap (conclude the lesson with a concise ONE-LINE summary of the strategy, rule, or key idea — the wrap never bridges to, previews, or mentions the mastery quiz or what comes next) → 9 language pass (LANG + the Mathematical Language Style Guide's preferred practice) and visual pass (VIS), tag every line, estimate timing from the GRADE words-per-minute → 10 self-QC against §17 citing rule IDs.

CONFLICT SCAN FIRST (§13.4 — conflicts are detected, AUTO-RESOLVED with the stack default, and recorded; generation NEVER pauses and NEVER asks): before writing anything, check the assembled inputs for conflicts — card-internal (approach vs. non-goals, objectives/evidence beyond the boundary or ceiling), card-vs-doctrine (the card's single strategy differs from the Stein format for this skill family), card-vs-playbook — the card against THIS RULEBOOK ('card-vs-playbook' is the wire kind; use it for rulebook conflicts: the Transfer Test cannot pass within sanity limits — a lesson genuinely needing more than 6:00 is a TIM 02 granularity flag; a needed interaction type is beyond MVP, INT 06; production exceeds the grade profile), and steering-vs-any (kind 'steering'). Resolve EVERY conflict yourself in this same reply, top-down through the authority stack with Stein — the DI book — STRICTLY SUPREME: card-vs-doctrine resolves to the Stein format's method (recorded, never silent). THE TOP-PRIORITY RESOLUTION RULE: when the DI book itself defers a topic, case, or skill to another week or a later point in its own sequence, the resolution is ALWAYS to defer that topic out of this lesson — a coverage-note deferral naming the downstream home plus a granularity/split signal back to the scope in qa.flags (TIM 02 style) — never to teach it here. Fill \`conflicts\` with one record per conflict you resolved (both sides with the exact card fields, doctrine format/page, and RULE IDS involved; \`proposal\` = the resolution you APPLIED; a one-line rationale) and WRITE THE FULL SCRIPT in the same reply with those resolutions applied — \`conflicts\` is a record attached to a complete script, NEVER a reason to emit an empty one. An auto-resolution may relax house defaults but never ADDS content beyond the card's Assessment Boundary or Non-Goals (SEQ 14 — Stein-mandated deferral REMOVES content, always allowed), never breaks the Transfer Test (SEQ 08), and never introduces a second strategy (SEQ 01). resolved_conflicts below are SETTLED from earlier runs: apply each resolution exactly and do not re-record them. RECORDED DEVIATIONS are settled house style and never flagged — DEV 01: expressions are read AS WRITTEN in symbol order ("2,352 divided by 42", LANG 10), deviating from Stein's divisor-first reading; inside algorithm steps the format's step wording still applies. DEV 02 (adopted with the Mathematical Language Style Guide): older-practice wording in a Stein format ("borrow", "carry the one") is recast in student-facing lines to the guide's preferred practice — wording only, strategy/steps/examples/corrections structurally Stein's; the DEV 01 algorithm-step reading is untouched; cite DEV 02 in a NOTE when applied.
