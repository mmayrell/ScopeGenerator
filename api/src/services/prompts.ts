...
1 standards — the official PARENT standard this atom is aligned to (canonical ID + normalized code) and, when the lesson teaches only part of it, the exact substandard/subpart taught (lettered sub-part or governing decomposition key). Nothing else. No citations on this field.
...
7 boundary — "Explicit Included/Excluded lists in consistent set vocabulary with concrete parameters; excluded content points to where it lives instead when known; components running on inference marked inferred."
...
9 approach — "Exactly one named strategy, selected per Stein (P3); then the modeling scope — cases explicitly modeled (the I Do → We Do set …) versus cases going straight to practice/extension." Begin new-learning approach fields with "Single strategy: <name>".
...
11 ceiling — "Concrete parameters — number sizes, step counts, representation load, context complexity — in consistent set vocabulary, naming the hardest legitimate case. Inferred ceilings marked, with what they extrapolate from."
...
Stage 2 — Scope Resolution: resolve the request to standards + governing decomposition keys (or the sub-part fallback), classify every supplied item per P2 against the governing standard's wording (contradiction detection per P1), and build the component evidence map. Apply D1 here: cap every component's difficulty at the highest observed tested difficulty and exclude content or difficulty the released evidence never assesses — log each cap and exclusion in scopeDecisions with its basis. Item absence NEVER drives structure: granularity comes solely from the A1–A6 procedure, and atoms or preskills that constitute the scoped standards stay in the plan at full quality even when no released item aligns to them directly (foundational and introductory atoms especially) — mark them evidenceStatus inferred; the cards fill the gap with generated assessment exemplars.
