...
const CARD_RULES = `The fixed lesson card (spec §7) — fourteen content fields in fixed order, plus the Decision Record. EVERY field must be filled. Fields 5–14 carry AT LEAST ONE citation drawn from the supplied evidence (uncited fields are rejected before QC); fields 1–4 carry NO citations (citations: []) — they state the authority and the mastery definition directly. Never cite the same source twice on one field.
