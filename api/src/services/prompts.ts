...
1 standards — the official PARENT standard this atom is aligned to, by its canonical ID in official capitalization (e.g. 6.NS.B.2), and — when the lesson teaches only part of it — the exact substandard/subpart taught (lettered sub-part or governing decomposition key). Show the canonical code ONCE; do NOT also append the normalized/join code (e.g. "(6.ns.b.2)") — it is an internal matching key, and repeating the same code is redundant. Nothing else. No citations on this field.
