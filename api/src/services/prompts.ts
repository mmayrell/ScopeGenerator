...
17 studentFriendlyTitle — a REQUIRED top-level lesson property (a sibling of title, never a card field): the lesson's title as a STUDENT will see it. As concise but descriptive as possible while remaining developmentally appropriate and on grade level: plain words a student in this course's grade reads and understands independently, naming what they will learn to do. Grade-level math vocabulary the lesson itself teaches or builds on is welcome ("area", "equivalent fractions"); standard codes, curriculum-internal vocabulary (atom, discrimination, representation-as-jargon, boundary, ceiling), and teacher-facing constraint language ("Within 1,000,000", "Using the Standard Algorithm" when the constraint only matters to sequencing) are not — carry a constraint only when the student would genuinely describe the lesson with it. Title Case. Sibling lessons' student titles must still be tellable apart. Example: engineering title "Divide Four-Digit Dividends by Two-Digit Divisors With Remainders" → student title "Dividing Big Numbers With Remainders".
...
- The opening: title portion ≤ 10 seconds naming the skill in STUDENT terms (TIM 03, LANG 11 — translate the card's title; no codes, no curriculum vocabulary), concisely set the goal. IF the card lists fragile prerequisites instructionally intrinsic to the first model: ONE quick retrieval check (SEQ 06 — skip prerequisites that are merely listed but unrelated to the opening task). THEN, if the lesson introduces a new rule, term, or notation: state it in plain words with one concrete example BEFORE the first problem (SEQ 15, LANG 05); a brief concrete hook fades before I Do (VIS 10). The student never meets a term for the first time inside a step.
- Every line carries exactly ONE channel (SAY | TEXT | VISUAL | INTERACTION | NOTE) per §14 — SAY in complete spoken sentences (LANG registry; pace per GRADE); TEXT numerals and short labels only (LANG 07); VISUAL one-canvas choreography (VIS 01–VIS 06: animate only the delta, exact synchronization, highlights fade, problem/steps/solution always visible); NOTE for timing marks, freeze frames, asset directions, and RULE-ID citations — never teaching content.
...
- Example selection: solved entirely by the taught strategy (SEQ 07, SEQ 11); demonstrations need NOT reach the difficulty ceiling (SEQ 09) — the video's obligation is to make ceiling transfer reasonable; released items are never reproduced (SEQ 13); the whole problem stays visible throughout (SEQ 08/VIS 03).
