...
**P12 — Sequence by Instructional Dependency; Keep Units Coherent; Interleave at the Margin.** Lessons are ordered by the DI sequence required for mastery, not by the order standards appear in a document or by keeping all atoms from the same standard together. Atoms aligned to the same standard may be separated across the course when prerequisite readiness, confusability, representation demands, or application demands require other lessons to come first. However, each atom must still sit inside a coherent instructional unit: it should belong to the unit's strand, build from nearby lessons, prepare for upcoming lessons, and make sense as part of the unit's visible skill chain. Ordering runs in two stages (engine Ordering rule 10, Interleaving After Dependency): the dependency graph is satisfied first and is non-negotiable; then, among the orders the graph admits, distributed interleaving is preferred over massed-topic blocks — a strand may be built as multiple internally coherent unit arcs alternating with other strands, retiring once it no longer supports future learning. Interleaving never violates prerequisite readiness, never separates tightly coupled dependencies, and never sacrifices coherent unit structure merely to increase alternation; significant sequencing choices are documented in the Decision Record against the dependency-valid alternatives.
...
- **Stage 4 — Sequencing & Unit Formation.** Doctrine-sourced ordering: preskills before composites; easier before difficult; algorithm before required representations; confusables separated in time, bridges only after both parents independently mastered; within a concept cluster, the sacrificial first instance — easiest sibling first with full modeling, later siblings as reduced-modeling transfer lessons. Units are strand-coherent (strand over spiral — though a strand may span multiple non-adjacent coherent unit arcs under Ordering rule 10's distributed interleaving; spiraling means thin scatter, not documented alternation), traceable to the set's theme/emphasis statements or progression streams, ordered by progression dependencies with high-weight strands anchoring the sequence. Granularity and count are purely logic-driven; no calendar constraint — students work self-paced, so the scope owes mastery structure, not pacing.
...
- Interoperability seam: card schema kept exportable (near-isomorphic to standards-extension structures such as CASE). The canonical JSON export (`src/export/scope-json.ts`) is the machine-readable deliverable, shaped as the course-operation envelope shared with Lesson Scope Generation (`LsgOutput`): `{ courseOperation, targetCourse: { courseId, courseName, grade, subject, standardSet }, lessons: [...] }`. Each lesson object carries `lessonId, operation, unitName, lessonOrder, standardId, lessonTitle, deactivationReason, objectives, assessmentBoundary, difficultyCeiling, prerequisites, progressionPlacement, newLearning, instructionalApproach, nonGoals, assessmentEvidence, releasedItems`, every field a string except `lessonId`/`deactivationReason` (null when absent) and `lessonOrder` (a 1-based running count across the whole course, not restarting per unit). A scope export is always a full course creation: `courseOperation` is `CREATE`, every lesson's `operation` is `CREATE`, `lessonId` and `deactivationReason` are null. Lesson Scope Generation runs download their output in the same envelope, with each lesson's own operation (`CREATE | UPDATE | DEACTIVATE`) as generated. Released items are structured references in text form (source, year, item number, aligned standard, alignment confidence, persistent screenshot URL), joined with blank lines into the `releasedItems` string; generated exemplars carry their full question text; Decision Records are maintained separately and are not exported.
