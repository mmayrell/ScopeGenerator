...
- Interoperability seam: card schema kept exportable (near-isomorphic to standards-extension structures such as CASE). The canonical JSON export (`src/export/scope-json.ts`) is the machine-readable deliverable: scope-level header (subject, course, standard set, generator version, generation date) plus per-lesson structured objects (standard, cluster, atomized objective, objectives, designation, placement, prerequisites, boundary, new learning, approach, non-goals, ceiling, assessment evidence, released-item references with time-limited screenshot URLs, generated exemplars, and per-field decision rationales).
...
The fourteen content fields of §7 plus the Decision Record, fixed order, fixed Title Case labels: Standard · Cluster · Substandard · Objectives · Major / Supporting · Progression Placement · Prerequisites · Assessment Boundary · New Learning · Instructional Approach · Non-Goals · Difficulty Ceiling · Assessment Evidence · Released Items (If Applicable) · Decision Record. Every field written under faultless communication — it must read one way only — and every field carries citations. A worked illustration (CCSS 4.NBT.5, "Multiply Up to a Four-Digit Number by a One-Digit Number, Standard Algorithm") ships as seed content in the reference implementation and is normative for the card's shape, illustrative in its content. (Seed lessons other than the flagship predate the Objectives field and render it as "—" until regenerated.)
